...
aiming to simultaneously minimize the preheating time (Ready Time) and the time on electrochemical heating phase (Work Time) as the two core objectives.
...
class FuelCellOptimizer:
    def __init__(self, random_seed=42, initial_weights=(-1.0, -1.0)):
        ...
        self.xgb_worktime_path = 'worktime_XGB_optimized.pkl'
        self.feature_names = [
        ...
        self.worktime_range = (30, 56)

        # 初始化模型和优化参数
        self.xgb_readytime = None
        self.xgb_worktime = None
        self.toolbox = None
    ...
    def load_models(self):
        """加载预训练模型"""
        model_files = [self.xgb_readytime_path, self.xgb_worktime_path]
        ...
        try:
            self.xgb_worktime = joblib.load(self.xgb_worktime_path)
            print("worktime_XGB_optimized model loaded successfully.")
        except Exception as e:
    ...
    def calculate_shap_importance(self):
        ...
        if self.xgb_readytime is None or self.xgb_worktime is None:
        ...
        explainer_work = shap.TreeExplainer(self.xgb_worktime)
    ...
    def denormalize_worktime(self, norm_value):
        """反归一化工作时间"""
        min_val, max_val = self.worktime_range
        return min_val + norm_value * (max_val - min_val)
    ...
    def evaluate(self, individual):
        ...
        try:
            ...
            y2_pred_norm = self.xgb_worktime.predict(X_test).flatten()[0]
        except Exception as e:
        ...
        y2_pred = self.denormalize_worktime(y2_pred_norm)

        # 添加约束惩罚 - 目标值低于下限时的惩罚
        penalty = 0.0
        if y1_pred < self.readytime_range[0]:
            penalty += (self.readytime_range[0] - y1_pred) * 10  # 惩罚系数
        if y2_pred < self.worktime_range[0]:
            penalty += (self.worktime_range[0] - y2_pred) * 10  # 惩罚系数
    ...
    def visualize_pareto_front(self, filename='pareto_front.png'):
        ...
        plt.axhline(y=self.worktime_range[0], color='green', linestyle='--', alpha=0.5, label='WorkTime下限')

        plt.xlabel('Ready Time (s)', fontsize=26, labelpad=10)
        plt.ylabel('Work Time (s)', fontsize=26, labelpad=10)
    ...
    def print_pareto_solutions(self):
        ...
        for i, ind in enumerate(self.pareto_front_individuals[:5]):  # 只打印前5个解
            ...
            print(f"  Ready Time: {ind.fitness.values[0]:.4f} seconds, Work Time: {ind.fitness.values[1]:.4f} seconds")
